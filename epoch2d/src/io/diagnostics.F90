...
CONTAINS
  ...
  SUBROUTINE average_field(ioutput, avg)
    ...
    IF (avg%dump_single) THEN
      ...
      CASE(c_dump_jx_diff)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) &
            + REAL(jx_diff(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
      CASE(c_dump_jy_diff)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) &
            + REAL(jy_diff(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
      CASE(c_dump_jz_diff)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) &
            + REAL(jz_diff(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
#endif        
#endif
      CASE(c_dump_ekbar)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
...
#ifdef APT_PLASMA
      CASE(c_dump_jx_diff)
        avg%array(:,:,1) = avg%array(:,:,1) + jx_diff(1-ng:nx+ng,1-ng:ny+ng) * dt
      CASE(c_dump_jy_diff)
        avg%array(:,:,1) = avg%array(:,:,1) + jy_diff(1-ng:nx+ng,1-ng:ny+ng) * dt
      CASE(c_dump_jz_diff)
        avg%array(:,:,1) = avg%array(:,:,1) + jz_diff(1-ng:nx+ng,1-ng:ny+ng) * dt
#endif        
#endif
      CASE(c_dump_ekbar)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
